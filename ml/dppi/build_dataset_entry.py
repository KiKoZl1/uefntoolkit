#!/usr/bin/env python3
"""Build DPPI entry dataset from feature store and labels."""

from __future__ import annotations

from pathlib import Path

from mlops import get_training_readiness, load_entry_dataset, upsert_dataset_meta
...
def main() -> None:
    args = parse_args_with_common("Build DPPI entry dataset", extra_args=_extra_args)
    ...
    with open_db() as conn:
        ...
        if frame.empty:
            raise RuntimeError("Entry dataset is empty. Ensure feature and label cron jobs are running.")

        frame["ts"] = frame["ts"].astype("datetime64[ns, UTC]")
        start_ts = frame["ts"].min().to_pydatetime()
        end_ts = frame["ts"].max().to_pydatetime()

        out_path = Path(args.output) if args.output else (cfg.artifacts_dir / "datasets" / f"entry_{cfg.region}_{cfg.surface_name}.parquet")
    ...
    print("[DPPI] entry dataset built")
