#!/usr/bin/env python3
"""Train DPPI entry model (CatBoost) with temporal split and readiness gate."""
...
def _extra_args(parser) -> None:
    parser.add_argument("--model-name", default="dppi_entry")
...
def main() -> None:
    args = parse_args_with_common("Train DPPI entry model", extra_args=_extra_args)
    cfg = RuntimeConfig.load(args.config, region=args.region, surface_name=args.surface_name, artifacts_dir=args.artifacts_dir)

    model_name = str(args.model_name or "dppi_entry")
    ...
    with open_db() as conn:
        ...
        if frame.empty:
            if run_id:
                finish_training_run(conn, run_id, status="failed", result_json={}, error_text="entry dataset empty")
            raise RuntimeError("entry dataset is empty")
        ...
        try:
            ...
            if split.train.empty or split.valid.empty or split.test.empty:
                raise RuntimeError("invalid temporal split for entry dataset")

            metrics_by_horizon: dict[str, dict[str, float]] = {}
            model_files: dict[str, str] = {}

            for horizon in cfg.entry_horizons:
            ...
            if not metrics_by_horizon:
                raise RuntimeError("no entry horizons could be trained")
    ...
    print("[DPPI] entry training finished")
