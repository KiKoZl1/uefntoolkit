...
-- Drop and recreate compute_discovery_public_intel with fixed permission check
DROP FUNCTION IF EXISTS public.compute_discovery_public_intel(TIMESTAMPTZ);
...
BEGIN
  -- Allow service_role JWT OR direct postgres/admin calls (cron)
  ...
  WITH dup_titles AS (
    SELECT ps.link_code,
      COALESCE(m.support_code, ic.creator_code, 'unknown') AS creator_code,
      COALESCE(m.title, ic.title) AS title,
    ...
    GROUP BY ps.link_code, creator_code, title, norm_title
  ),
  clusters AS (
    SELECT creator_code, norm_title, COUNT(DISTINCT link_code) AS island_count,
      array_agg(DISTINCT title ORDER BY title) AS titles
    FROM dup_titles WHERE norm_title IS NOT NULL AND norm_title <> ''
    GROUP BY creator_code, norm_title HAVING COUNT(DISTINCT link_code) >= 2
  ),
  per_creator AS (
    SELECT creator_code, COUNT(*)::int AS duplicate_clusters_7d,
    ...
    FROM clusters GROUP BY creator_code
  )
