-- Apply one exposure tick atomically (raw + segments + tick row update) in a single transaction.
...
BEGIN
  ...
  -- Normalize rows into a temp CTE
  WITH incoming AS (
    SELECT
    ...
    FROM jsonb_to_recordset(COALESCE(p_rows, '[]'::jsonb)) AS x(
      ...
      is_visible boolean,
      lock_status text,
      lock_status_reason text
    )
    WHERE x.panel_name IS NOT NULL
      AND x.panel_name <> ''
      AND x.link_code IS NOT NULL
      AND x.link_code <> ''
  ),
  counts AS (
    SELECT
      COUNT(*)::int AS entries_count,
      COUNT(DISTINCT panel_name)::int AS panels_count
    FROM incoming
  )
  SELECT entries_count, panels_count INTO v_entries_count, v_panels_count FROM counts;

  -- 1) Raw insert (debug retention only; still atomic with segments)
  ...
  -- 2) Presence segments (one sample per (panel, link_code) per tick)
  ...
  SELECT
    ...
    i.global_ccu, i.global_ccu, i.global_ccu,
    NULL
  ...
  DO UPDATE SET
    ...
    best_rank = LEAST(COALESCE(public.discovery_exposure_presence_segments.best_rank, EXCLUDED.best_rank), EXCLUDED.best_rank),
    rank_sum = public.discovery_exposure_presence_segments.rank_sum + EXCLUDED.rank_sum,
    rank_samples = public.discovery_exposure_presence_segments.rank_samples + 1,
    end_rank = EXCLUDED.end_rank,
    ccu_end = EXCLUDED.ccu_end,
    ccu_max = CASE
      WHEN public.discovery_exposure_presence_segments.ccu_max IS NULL THEN EXCLUDED.ccu_max
      WHEN EXCLUDED.ccu_max IS NULL THEN public.discovery_exposure_presence_segments.ccu_max
      ELSE GREATEST(public.discovery_exposure_presence_segments.ccu_max, EXCLUDED.ccu_max)
    END,
    closed_reason = NULL;
  GET DIAGNOSTICS v_presence_upserted = ROW_COUNT;

  -- Close presence segments that were not seen in this tick (no invented gaps)
  WITH incoming_presence_keys AS (
    SELECT DISTINCT panel_name, link_code
    FROM incoming
  )
  UPDATE public.discovery_exposure_presence_segments s
  SET end_ts = s.last_seen_ts,
      closed_reason = 'absent_confirmed'
  WHERE s.target_id = p_target_id
    AND s.end_ts IS NULL
    AND NOT EXISTS (
      SELECT 1 FROM incoming_presence_keys k
      WHERE k.panel_name = s.panel_name AND k.link_code = s.link_code
    );
  GET DIAGNOSTICS v_presence_closed = ROW_COUNT;

  -- 3) Rank segments
  -- Close "replaced" ranks first so inserts won't conflict.
  WITH incoming_rank_keys AS (
    SELECT
      surface_name, panel_name,
      MAX(panel_display_name) AS panel_display_name,
      MAX(panel_type) AS panel_type,
      ...
      MAX(global_ccu)::int AS global_ccu
    FROM incoming
    GROUP BY surface_name, panel_name, rank, link_code
  )
  ...
  WHERE s.target_id = p_target_id
    AND s.end_ts IS NULL
    AND EXISTS (
      SELECT 1 FROM incoming_rank_keys k
      WHERE k.panel_name = s.panel_name
        AND k.rank = s.rank
        AND k.link_code <> s.link_code
    );
  ...
  DO UPDATE SET
    ...
    ccu_end = EXCLUDED.ccu_end,
    ccu_max = CASE
      WHEN public.discovery_exposure_rank_segments.ccu_max IS NULL THEN EXCLUDED.ccu_max
      WHEN EXCLUDED.ccu_max IS NULL THEN public.discovery_exposure_rank_segments.ccu_max
      ELSE GREATEST(public.discovery_exposure_rank_segments.ccu_max, EXCLUDED.ccu_max)
    END,
    closed_reason = NULL;
  GET DIAGNOSTICS v_rank_upserted = ROW_COUNT;

  -- Close open ranks that were absent in this tick
  WITH incoming_rank_keys AS (
    SELECT DISTINCT panel_name, rank
    FROM incoming
  )
  UPDATE public.discovery_exposure_rank_segments s
  SET end_ts = s.last_seen_ts,
  ...
  -- 4) Tick telemetry row update
  UPDATE public.discovery_exposure_ticks
  SET ts_end = now(),
      status = 'ok',
      branch = p_branch,
      test_variant_name = p_test_variant_name,
      test_name = p_test_name,
      test_analytics_id = p_test_analytics_id,
  ...
  RETURN jsonb_build_object(
    ...
    'presence_upserted', v_presence_upserted,
    'presence_closed', v_presence_closed,
    'rank_replaced_closed', v_rank_replaced_closed,
    'rank_upserted', v_rank_upserted,
