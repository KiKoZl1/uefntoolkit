...
BEGIN
  ...
  next_dedup_window AS (
    SELECT
      ...
      MIN(gap_minutes) AS gap_minutes
    FROM next_after_close
    WHERE transition_ts >= v_window_start
    GROUP BY source_panel, next_panel, link_code
  ),
  next_counts_24h AS (
    SELECT
  ...
  next_counts_window AS (
    SELECT
      source_panel,
      next_panel,
      COUNT(*)::int AS cnt,
      PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY gap_minutes) AS gap_p50
    FROM next_dedup_window
    GROUP BY source_panel, next_panel
  ),
  next_totals AS (
    SELECT
  ...
  next_ranked AS (
    SELECT
      ...
      n.cnt,
      n.gap_p50,
      SUM(n.cnt) OVER (PARTITION BY n.source_panel) AS total_cnt_window,
      ROW_NUMBER() OVER (PARTITION BY n.source_panel ORDER BY n.cnt DESC, n.next_panel ASC) AS rn
    FROM next_counts_window n
  ),
  next_agg AS (
    SELECT
      ...
      COALESCE(
        jsonb_agg(
          jsonb_build_object(
            ...
            'share_pct',
              CASE
                WHEN r.total_cnt_window >= 8 THEN ROUND((r.cnt::numeric * 100.0) / r.total_cnt_window, 1)
                ELSE NULL
              END,
  ...
  prev_counts_window AS (
    SELECT
      dest_panel,
      prev_panel,
      COUNT(*)::int AS cnt,
      PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY gap_minutes) AS gap_p50
    FROM prev_dedup_window
    GROUP BY dest_panel, prev_panel
  ),
  prev_totals AS (
    SELECT
  ...
  prev_ranked AS (
    SELECT
      ...
      p.cnt,
      p.gap_p50,
      SUM(p.cnt) OVER (PARTITION BY p.dest_panel) AS total_cnt_window,
      ROW_NUMBER() OVER (PARTITION BY p.dest_panel ORDER BY p.cnt DESC, p.prev_panel ASC) AS rn
    FROM prev_counts_window p
  ),
  prev_agg AS (
    SELECT
