...
BEGIN
  ...
  IF v_cycle.id IS NOT NULL THEN
    IF v_cycle.plan_type = 'free'
      AND v_account.free_signup_granted_at IS NULL
      AND v_account.free_eligible
      AND NOT v_account.anti_abuse_review_required
      AND v_account.access_state <> 'blocked_abuse_review' THEN
      v_free_grant := public._commerce_cfg_int(
        'free_signup_credits',
        public._commerce_cfg_int('free_monthly_credits', 80)
      );

      IF v_free_grant > 0 AND COALESCE(v_cycle.free_monthly_grant, 0) = 0 THEN
        UPDATE public.commerce_billing_cycles
        SET
          free_monthly_grant = v_free_grant,
          updated_at = now()
        WHERE id = v_cycle.id;
        ...
        SET
          ...
          free_monthly_remaining = free_monthly_remaining + v_free_grant,
        ...
        SET
          ...
          free_signup_grant_credits = GREATEST(free_signup_grant_credits, v_free_grant),
          updated_at = now()
        WHERE user_id = p_user_id;

        v_key_prefix := format('%s:%s:%s', p_idempotency_prefix, p_user_id::text, v_cycle.id::text);
        PERFORM public.commerce_insert_ledger(
          p_user_id, v_cycle.id, 'free_monthly', 'free_monthly_grant', v_free_grant, NULL,
          v_operation_id, v_cycle.id::text, v_key_prefix || ':free_signup_catchup', 'free_signup_grant_catchup', '{}'::jsonb, NULL, 'system'
        );
      END IF;
    END IF;
