...
import { ArrowLeftRight, LogOut, Menu, Shield, User } from "lucide-react";
import { useTranslation } from "react-i18next";
import { Button } from "@/components/ui/button";
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetTrigger } from "@/components/ui/sheet";
import { cn } from "@/lib/utils";
import { NavAccessState, NavSection, TopBarContext } from "@/navigation/types";
import { isNavItemActive, isNavItemProtectedForAccess } from "@/navigation/config";
...
export const MobileTopNav = memo(function MobileTopNav({ context, sections, access, onSignOut }: MobileTopNavProps) {
  ...
  return (
    <>
          ...
          <SheetHeader className="border-b border-border/60 px-5 pb-4">
            <SheetTitle className="flex items-center justify-between">
              <Link to="/" onClick={() => setOpen(false)}>
                <PlatformBrand className="gap-2" iconClassName="h-8 w-8" textClassName="text-base" />
              </Link>
                      ...
                      if (protectedForAccess) {
                        return (
                          <button
                            ...
                            onClick={() => {
                              setOpen(false);
                              setAuthGate({ open: true, label: t(item.labelKey) });
                            }}
                            className={cn(
                              "nav-motion-base block w-full rounded-lg border px-3 py-2.5 text-left transition-[background-color,color,border-color] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {(access.isAdmin || access.isEditor) && (
                      <Button variant="outline" className="justify-start" asChild>
                        <Link to={contextSwitchTo} onClick={() => setOpen(false)}>
      ...
      <AuthGateDialog
        open={authGate.open}
        onOpenChange={(nextOpen) => setAuthGate((prev) => ({ ...prev, open: nextOpen }))}
