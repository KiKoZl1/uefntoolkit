...
test("all declared routes latency map (admin authenticated)", async ({ page, baseURL }) => {
  ...
  for (const route of routes) {
    const t0 = Date.now();
    const response = await page.goto(`${root}${route}`, { waitUntil: "domcontentloaded", timeout: 120_000 });
    ...
    const api = await page.evaluate(() => {
      ...
      return { totalCalls: calls.length, maxDuration: durations.length ? Math.max(...durations) : 0 };
    });
  ...
  fs.writeFileSync(".tmp/perf/routes_all_latency_admin.json", JSON.stringify(out, null, 2), "utf8");
  console.log("ROUTES_ALL_LATENCY_ADMIN=" + JSON.stringify(out));
});
